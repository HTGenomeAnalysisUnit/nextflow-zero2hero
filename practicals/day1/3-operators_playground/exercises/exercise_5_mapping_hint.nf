...
workflow {
	...
	// TODO 1: Use regex to extract metadata from file names
	regex_map_channel = neo_latin_greetings_files_collect_file
		.map{neo_latin_greeting ->
			// YOUR CODE HERE - use regex to extract family name from file path
			// Return tuple with extracted family metadata and the file
		}

	// TODO 2: Create multiple channels with different structures using multiMap
	greetings_multimap = greeting_files
		.multiMap{meta, greeting ->
			// YOUR CODE HERE - create two different channel structures:
